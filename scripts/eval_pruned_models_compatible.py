...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory where the pruning results were stored. "
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a specific pruning mask file to evaluate. If provided, the script will evaluate only this mask.",
    )
    parser.add_argument(
        "--sparsity",
        type=float,
        default=None,
        help=(
            "Sparsity level associated with the provided --mask-file. "
            "This is used for logging and result interpretation purposes."
        ),
    )
...
def main():
    ...
    # Initialize second_evaluator and macs_evaluator if needed
    ...
    # Define a helper function to process a single mask file
    def process_mask(mask_path: str, sparsity: float = None):
        nonlocal model, dump_data_list
        ...
        logger.info(
            f"Total #params in pruned model: {num_params}, pruning ratio: {(1 - num_params / ori_total_param_count):.4f}"
        )
        ...
        if cfg.test_cfg.get("eval_original", True) and original_mean_time is not None and sparsity is not None:
    ...
    # If a specific mask file is provided, evaluate only that mask
    if args.mask_file is not None:
        if not osp.exists(args.mask_file):
            raise FileNotFoundError(f"The specified mask file does not exist: {args.mask_file}")
        if args.sparsity is None:
            # Optionally, try to infer sparsity from the filename or raise an error
            logger.warning(
                "Sparsity level is not provided. You should specify it using --sparsity for accurate logging."
            )
            inferred_sparsity = None
        else:
            inferred_sparsity = args.sparsity
        process_mask(args.mask_file, sparsity=inferred_sparsity)
    else:
        # Evaluate masks based on sparsity levels from config
        for sparsity in cfg.test_cfg.sparsities:
            mask_path = osp.join(
                args.pruning_dir,
                "pruning_masks",
                f'{args.mask_prefix}_{str(sparsity).replace(".", "_")}_pruning_masks.pth',
            )
            process_mask(mask_path, sparsity=sparsity)
